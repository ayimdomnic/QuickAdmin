...
@section('content')
    <p class="login-box-msg">{{ trans('quickadmin.password.intro') }}</p>
    ...
    <form role="form" method="POST" action="{{ action($ctrl_name.'@postEmail') }}">
        ...
        <button type="submit" class="btn btn-primary btn-block btn-flat">{{ trans('quickadmin.password.reset') }}</button>
    </form>
    @stop